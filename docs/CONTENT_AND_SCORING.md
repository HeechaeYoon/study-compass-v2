...
```ts
{
  ...
  type: "scenario",
  axisLabel: "실행",
  text: "최근 공부할 때, 나는 주로 어떤 방식으로 집중을 이어가나요?",
  options: [
    {
      id: "A",
      text: "시작해도 금방 다른 일로 넘어가 집중이 끊겨요.",
      dotLevel: 1,
      scores: { P: 0, E: 0, U: 0, M: 0, H: 0 },
    },
    {
      id: "B",
      text: "짧은 시간은 해보지만 중간 확인은 자주 놓쳐요.",
      dotLevel: 2,
      scores: { P: 0, E: 1, U: 0, M: 1, H: 0 },
    },
    {
      id: "C",
      text: "상황에 따라 달라져서 목표나 환경을 다시 맞춰요.",
      dotLevel: 3,
      scores: { P: 1, E: 2, U: 0, M: 2, H: 0 },
    },
    {
      id: "D",
      text: "정한 시간 동안 이어가고 끝나면 한 번 확인해요.",
      dotLevel: 4,
      scores: { P: 1, E: 3, U: 0, M: 2, H: 0 },
    },
    {
      id: "E",
      text: "몰입해서 분량을 마치고 막힌 부분도 표시해요.",
      dotLevel: 5,
      scores: { P: 0, E: 4, U: 0, M: 3, H: 0 },
    },
  ],
}
```
...
```ts
{
  ...
  options: [
    {
      id: "A",
      text: "오늘 끝낼 분량을 작게 정하고 바로 시작할 준비를 한다.",
      scores: { P: 3, E: 1, U: 0, M: 1, H: 0 },
    },
    {
      id: "B",
      text: "쉬운 문제 하나를 풀면서 흐름을 만든다.",
      scores: { P: 1, E: 3, U: 0, M: 1, H: 0 },
    },
    {
      id: "C",
      text: "지난 설명이나 예시를 보고 오늘 배울 내용을 내 말로 떠올린다.",
      scores: { P: 0, E: 0, U: 3, M: 1, H: 1 },
    },
    {
      id: "D",
      text: "아는 것과 헷갈리는 것을 표시하고 확인할 순서를 정한다.",
      scores: { P: 1, E: 0, U: 1, M: 3, H: 0 },
    },
    {
      id: "E",
      text: "혼자 잠깐 살펴본 뒤 필요한 자료나 질문할 곳을 정한다.",
      scores: { P: 1, E: 0, U: 0, M: 1, H: 4 },
    },
  ],
}
```
...
```ts
{
  ...
  options: [
    {
      id: "A",
      text: "조건, 구해야 할 것, 풀이 순서를 나누어 본다.",
      scores: { P: 3, E: 0, U: 0, M: 1, H: 0 },
    },
    {
      id: "B",
      text: "완벽하지 않아도 내가 아는 부분부터 써본다.",
      scores: { P: 0, E: 3, U: 1, M: 1, H: 0 },
    },
    {
      id: "C",
      text: "비슷한 예시를 보고 왜 그런 풀이가 되는지 다시 설명해본다.",
      scores: { P: 0, E: 0, U: 3, M: 0, H: 1 },
    },
    {
      id: "D",
      text: "막힌 줄을 표시하고 실수인지 개념인지 이유를 찾는다.",
      scores: { P: 0, E: 0, U: 1, M: 4, H: 0 },
    },
    {
      id: "E",
      text: "내가 시도한 부분과 막힌 부분을 적어 도움을 요청한다.",
      scores: { P: 0, E: 1, U: 0, M: 1, H: 4 },
    },
  ],
}
```
...
```ts
{
  ...
  options: [
    {
      id: "A",
      text: "남은 분량을 보고 다음 공부 순서를 짧게 고친다.",
      scores: { P: 3, E: 0, U: 0, M: 1, H: 0 },
    },
    {
      id: "B",
      text: "정한 분량을 끝냈는지 보고 조금 더 이어갈지 정한다.",
      scores: { P: 1, E: 3, U: 0, M: 1, H: 0 },
    },
    {
      id: "C",
    ...
    {
      id: "D",
      text: "헷갈린 것과 오답 원인을 표시하고 다음 확인 문제를 정한다.",
      scores: { P: 1, E: 0, U: 1, M: 4, H: 0 },
    },
    {
      id: "E",
      text: "남은 질문을 한 줄로 정리해 자료나 사람에게 확인한다.",
      scores: { P: 0, E: 0, U: 0, M: 1, H: 4 },
    },
  ],
}
```
...
1. 모든 기준 프로필과 거리를 먼저 비교한다.
2. 가까운 프로필 순서대로 guardrail을 통과하는 첫 프로필을 대표 성향으로 사용한다.
3. `balanced_coordinator`는 전체 평균 `>= 45`, 표준편차 `< 15`, 모든 축 `>= 30`일 때만 허용한다.
4. `foundation_builder`는 전체 평균 `< 32`, 최고 축 `< 55`일 때만 허용한다.
5. 그 외 성향은 별도 guardrail 없이 거리 순위를 따른다.

이 규칙은 `balanced_coordinator`와 `foundation_builder`가 다른 최근접 프로필을 앞에서 가로채지 않도록 하기 위한 보정이다. 응답공간 점검에서 `balanced_coordinator`를 평균 `>= 55`, 모든 축 `>= 40`으로 제한하면 조건 자체를 만족하는 조합이 3%에 크게 못 미치므로, v1은 "중간 이상으로 고르게 나타나는 균형"을 잡는 완화 guardrail을 사용한다.
...
이 스크립트는 전체 응답 조합 `4^12 * 5^4 = 10,485,760,000`개를 원점수 상태로 압축한 뒤, 실제 `matchLearningType()`을 적용해 분포를 계산한다.
...
- 모든 유형은 전체 조합 기준 3% 이상이어야 한다.
- 단일 유형은 30%를 넘지 않아야 한다.
- `foundation_builder`는 30% 이하이어야 한다.
- `balanced_coordinator`는 22% 이하이어야 한다.
...
1. 점수 70 이상인 축을 내림차순
2. 최대 2개
3. 70 이상이 없다면 상위 2개를 `먼저 활용할 전략` 후보로 사용한다.
4. 동점이면 축 순서 `P, E, U, M, H`

화면 문구에서 `강점`이라는 제목은 실제 `강점` 라벨 축이 있을 때만 사용한다.
...
- 요약: `공부를 시작하기 전에 방향과 순서를 잡는 전략이 잘 맞을 수 있어요.`
- 설명: `공부를 시작하기 전에 길을 먼저 그리는 지도 제작자처럼 목표와 순서를 정할 때 흐름이 안정될 수 있습니다.`
- 강점:
  - 목표와 순서를 정리하기 좋습니다.
...
- 설명: `학습 결과를 확인하고 다음 방법을 고치는 전략이 잘 맞을 수 있습니다.`
- 강점:
  - 오답 원인 찾기
  - 공부 방법 수정
  - 작은 테스트 활용
...
- 실제 `강점` 라벨 축이 있으면 카드 제목은 `강점`이다.
- 실제 `강점` 라벨 축이 없으면 카드 제목은 `먼저 활용할 전략`이다.
- 실제 `균형` 라벨 축이 있으면 카드 제목은 `균형`이다.
- 실제 `균형` 라벨 축이 없으면 카드 제목은 `다음 점검`이다.
- 결과 첫 화면과 이미지 내보내기의 성향 요약은 `createResultSummary().typeSummary`를 사용한다. 실제 `강점` 라벨 축이 없으면 성향별 고정 요약 대신 `아직 특정 축을 단정하기보다 ... 축을 먼저 활용`하는 조건부 문구를 보여준다.
- 상세 리포트와 AI 프롬프트에는 `왜 이렇게 봤나요?` 또는 `이렇게 판단한 근거` 섹션을 넣어 상위 축, 성장 축, 선택한 상황형 답변 1~2개를 보여준다.
- 상황형 답변 근거는 선택지 원문을 그대로 인용하지 않는다. 특히 낮은 응답의 `집중이 끊겨요`, `자주 놓쳐요` 같은 표현이 결과에 반복되지 않도록, 선택된 행동을 중립적인 코칭 문장으로 요약한다.
...
빈 값은 `아직 입력하지 않았습니다`로 처리하거나 해당 문장을 자연스럽게 생략한다. 대괄호 placeholder를 그대로 복사하지 않는다. `memo`는 비어 있지 않으면 별도 체크박스 없이 프롬프트에 자동 포함한다.
...
- `공부 계획`: 30~40분 안에 실행 가능한 4~6단계 계획, 도움 요청 방법, 자기점검 질문 3개, 후속 질문 2개를 요청한다.
- `개념 학습`: 핵심 개념 3~5개, 쉬운 예시나 비유, 오개념·실수 포인트, 확인문제 3개, 간단한 채점 기준을 요청한다.
- `계획 이미지`: Gemini 또는 ChatGPT 이미지 생성 기능에 붙여넣을 학습 계획 인포그래픽 프롬프트를 만든다.
- `개념 이미지`: Gemini 또는 ChatGPT 이미지 생성 기능에 붙여넣을 개념 학습 인포그래픽 프롬프트를 만든다.
- 이미지 프롬프트 모드는 문항과 답변을 디자인 참고 맥락으로만 쓰고 이미지에 그대로 쓰지 말라고 명시한다.
- 이미지 프롬프트 모드는 이름, 연락처, 민감한 개인정보, 원문 답변, 점수, 고정된 성향 라벨을 이미지에 넣지 말라고 명시한다.
...
```text
메모를 입력하면 프롬프트에 함께 반영돼요. 이름, 연락처, 민감한 개인정보는 적지 않는 것이 좋아요.
```
